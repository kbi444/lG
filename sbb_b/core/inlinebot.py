...
CATLOGO = "https://telegra.ph/file/2af327cf756bd244e630d.jpg"
...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧⵧⵧLG ⵧⵧⵧⵧⵧ⍣**\n[ LG 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/sorsLG)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**sbb_b"):
            buttons = [
                (
                    ...
                    Button.url("DEV", "https://t.me/sorsLG"),
                )
            ]
            ...
            result = builder.article(
                title="ارسال همسه سرية الى 👀",
                text=f"🔒 هذه همسه سرية الى {RAZAN}, هو فقط من يستطيع رؤيتها",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            result = builder.article(
                title="sbb_b Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب LG **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/sorsLG"),
                Button.url(
                    "المطور",
                    "t.me/sorsLGg",
                ),
        ...
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[LG 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/sorsLG)",
