...
ANTI_DDDD_olga_MODE = ChatBannedRights(
    until_date=None,
    view_messages=None,
    send_media=True,
    send_stickers=True,
...
async def _(event):
    res = ""
    current_olga_locks = get_locks(event.chat_id)
    if not current_olga_locks:
        res = "**• اعدادات اوامر الحماية في المجموعة:**"
    else:
        res = "**- اعدادات اوامر الحماية في المجموعة::** \n"
        ubots = "❌" if current_olga_locks.bots else "✅"
        uegame = "❌" if current_olga_locks.egame else "✅"
        urtl = "❌" if current_olga_locks.rtl else "✅"
        uforward = "❌" if current_olga_locks.forward else "✅"
        ubutton = "❌" if current_olga_locks.button else "✅"
        uurl = "❌" if current_olga_locks.url else "✅"
        ugame = "❌" if current_olga_locks.game else "✅"
        ulocation = "❌" if current_olga_locks.location else "✅"
        ucontact = "❌" if current_olga_locks.contact else "✅"
        ubutton = "❌" if current_olga_locks.button else "✅"
        uinline = "❌" if current_olga_locks.inline else "✅"
        res += f"**•  البوتات :** {ubots}\n"
    ...
    if is_locked(chat_id, "game") and event.message.media:
        if R0R77 == razan or await is_admin(event, R0R77) or R0R77 in devs:
            ...
            try:
                ...
                await event.client(
                    EditBannedRequest(
                        event.chat_id, event.sender_id, ANTI_DDDD_olga_MODE
                    )
                )
            except Exception as e:
                await event.reply(
...
async def _(event):
    ...
    if event.user_added:
        ...
        olga = await event.client.get_permissions(event.chat_id, jasem)
        is_ban_able = False
        ...
        for user_id in added_users:
            ...
            if event.user_added:
                is_ban_able = True
                if jasem == razan or olga.is_admin or jasem in devs:
                    return
                else:
                    try:
                        await event.client(
...
async def _(event):
    ...
    if event.user_joined:
        ...
        theolga = await event.client.get_entity(event.user_id)
        is_ban_able = False
        rights = types.ChatBannedRights(until_date=None, view_messages=True)
        if event.user_joined and (
            ...
            is_ban_able = True
            if theolga.id in devs:
                return
            else:
                try:
                    await event.client(
                        functions.channels.EditBannedRequest(
                            event.chat_id, theolga.id, rights
                        )
                    )
                    await event.reply(
...
async def r0r77(olga):
    con = olga.pattern_match.group(1).lower()
    del_u = 0
    del_status = "**• مجموعتك/قناتك في أمان ✅ لاتوجد بوتات في هذه المجموعة**"
    if con != "طرد":
        event = await edit_or_reply(olga, "**• جاري البحث عن بوتات في هذه المجموعة**")
        async for user in olga.client.iter_participants(olga.chat_id):
            if user.bot:
                del_u += 1
                await sleep(0.5)
        if del_u > 0:
    ...
    chat = await olga.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    if not admin and not creator:
        await edit_delete(olga, "**• عذرا يجب أن اكون مشرف في هذه المجموعة اولا**", 5)
        return
    event = await edit_or_reply(olga, "**• جارِ طرد البوتات من هنا أنتظر قليلا**")
    del_u = 0
    del_a = 0
    async for user in olga.client.iter_participants(olga.chat_id):
        if user.bot:
            try:
                await olga.client.kick_participant(olga.chat_id, user.id)
                await sleep(0.5)
                del_u += 1
    ...
    if BOTLOG:
        await olga.client.send_message(
            BOTLOG_CHATID,
            f"#طرد_البوتات\
            \n • {del_status}\
            \n • الدردشه: {olga.chat.title}(`{olga.chat_id}`)",
        )
