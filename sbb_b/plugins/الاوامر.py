...
async def hi(event):
    await edit_or_reply(
        event,
        "[ 𝐒𝗈𝐮𝐫𝐜𝐞 LG ](t.me/sorsLG) اوامـر جلب الذاتية وامر الاضافة**:\n\n ٭ 𝅄ׄ  `.ضيف`\n—اكتب الامر مع رابط مجموعه التي تريد سحب اعضائها وارسل الامر في مجموعتك لسحبهم الى مجموعتك\n\n ⟸`.جلب الصورة`  او  `.ذاتية`\n—بالرد على صورة او فيديو ذاتية التدمير لحفظها وارسالها في الرسائل المحفوظة بسرية تامة الامر لLG حصريا!",
    )
...
async def we(event):
    await edit_or_reply(
        event,
        "**[ 𝐒𝗈𝐮𝐫𝐜𝐞 LG ](t.me/sorsLG)**\n▭▭▭▭▭▭▭▭▭▭▭▭▭▭\n\n• اوامـر الـــتـحـكـم تتيح لمستخدم اخر ان يستخدم اوامـر تنصيبك وهو غير منصب LG حيث سيصبح متحكم في اوامـر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامـر وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.الـــتـحـكـم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية الـــتـحـكـم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب LG الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين",
        link_preview=False,
    )
