...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, olga, sleeptimem, sleeptimet, DelaySpam=True)


@sbb_b.ar_cmd(pattern="تفاعلات$")
async def react_spam(event):
    msg = await event.get_reply_message()
    if not msg:
        return await edit_delete(event, "**- يجب عليك الرد على الرسالة اولا**", 10)
    olgaevent = await edit_or_reply(event, "**- جار بدأ العملية انتظر قليلا**")
