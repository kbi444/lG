...
async def hi(event):
    await edit_or_reply(
        event,
        "**[ . LG - 𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒 .](t.me/sorsLG)\n\n٭ 𝅄ׄ  اوامر التحكم تتيح لمستخدم اخر ان يستخدم السورس بدون تنصيبه حيث سيصبح متحكم في تنصيبك، يرجى الانتباه من انه سيحصل على صلاحيات الأوامر  وقد يسبب خطورة على حسابك، اذا كنت لا تثق في المستخدم الذي اضفته.\n\n٭ 𝅄ׄ  `.التحكم` تفعيل/تعطيل\n٭ 𝅄ׄ  تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n—`.اضف متحكم`\n٭ 𝅄ׄ  بالرد على المستخدم لأضافته متحكم في تنصيب LG الخاص بك\n\n—`.ازالة متحكم`\n٭ 𝅄ׄ  بالرد على المستخدم لازالته من قائمة المتحكمين\n\n—`.المتحكمين`\n٭ 𝅄ׄ لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين**",
        link_preview=False,
    )
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "**• يبدو انك لم تقوم بأضافة مستخدم متحكم LG الخاص بك**"
        )
    result = "**قائمة المستخدمين المتحكمين بتنصيب LG :**\n\n"
