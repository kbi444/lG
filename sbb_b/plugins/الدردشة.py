...
async def _(event):
    ...
    olgaevent = await edit_or_reply(event, "**بوياي جار**")
...
async def _(event):
    olgaevent = await edit_or_reply(event, "**- جار الغاء حظر جميع المستخدمين**")
    succ = 0
    total = 0
    flag = False
...
async def rm_deletedacc(show):
    ...
    if con != "تنظيف":
        ...
        if flag != " -r":
            ...
            olgaadmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not olgaadmin:
