...
async def permalink(mention):
    ...
    if user.id == 6897438263:
        return await edit_or_reply(mention, f"**- متزوجة قبل مالك دخل فيها**")
    if custom:
        return await edit_or_reply(mention, f"[{custom}](tg://user?id={user.id})")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
...
@sbb_b.ar_cmd(pattern="اقتباس(?: |$)(.*)")
async def mention(mention):
    quo = random.choice(quotes)
    await edit_or_reply(mention, f"**- {quo}**")
...
@sbb_b.on(admin_cmd(pattern="طالق(?:\s|$)([\s\S]*)"))
async def mention(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
