...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**[ 𝐒𝗈𝐮𝐫𝐜𝐞 LG ](t.me/sorsLG)**"
...
def olgaalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    olga_caption = "『  LG 𝙸𝚂 𝚆𝙾𝚁𝙺𝙸𝙽𝙶 ✓  』*\n"
    olga_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    olga_caption += f"**{EMOJI} اصدار LG :** `{olgaversion}`\n"
...
async def repo(event):
    ...
    response = await sbb_b.inline_query(sorsLGg, "السورس")
    await response[0].click(event.chat_id)
    await event.delete()

ROZ_PIC = "https://telegra.ph/file/2af327cf756bd244e630d.jpg"
        ...
        if query.startswith("السورس") and event.query.user_id == sbb_b.uid:
            buttons = [
                [
                    ...
                    Button.url("المطـور 👨🏼‍💻", "https://t.me/sorsLGg"),
                ]
            ]
