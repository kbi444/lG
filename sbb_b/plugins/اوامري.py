...
ROE = "** ꒱ ِ  هَذه هي قائمة اوامـر سورس LG   𓏲 :**"
ROZADM = "من هنا يمكنك ايجاد جميع الأوامر"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == sbb_b.uid:
            buttons = [
                [Button.inline("꒱ ِمعلومات LG  𓏲 :", data="AOMRDB")],
                [
                    Button.inline("꒱  البوت  𓏲 :", data="eeeeq"),
                    Button.inline("꒱  القروب  𓏲 :", data="admincmd_s")
                ],
                [
                    Button.inline("꒱  التسلية  𓏲 :", data="TASLIACMD"),
                    Button.inline("꒱  الاداوات  𓏲 :", data="toolsed1"),
                ],
                [
                    Button.inline("꒱  التحميل  𓏲 :", data="DOWMANLODE4"),
                ],
                [
                    Button.inline("꒱  الاكسترا  𓏲 :", data="EXTRACMD"),
                    Button.inline(" ꒱  الفارات  𓏲 :", data="VARolga"),
                ]
            ]
...
@sbb_b.tgbot.on(CallbackQuery(data=re.compile(rb"VARolga")))
...
async def varssett(event):
    await event.edit(
        ...
        buttons=[
            ...
            [
                Button.inline("رجوع", data="VARolga")
            ],
            [
                Button.inline("القائمة الرئيسية", data="MAIN")
            ]
...
* يمكنك الحصول على  كليشة جاهزة من هذه القناة @sorsLG
ملاحظة : يمكنك استخدام الاوامر في اي دردشة او محادثة
اوامر فارات سورس LG @sorsLG""",
        buttons=[
            [Button.inline("رجوع", data="pmvars")],
