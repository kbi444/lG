...
async def promote(event):
    ...
    if not user:
        return
    olgaevent = await edit_or_reply(event, "** جار رفع المستخدم يرجى الانتظار**")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await olgaevent.edit(NO_PERM)
    await olgaevent.edit("**- تم بنجاح رفع المستخدم مشرف في الدردشة**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"رفع مشرف\
...
async def demote(event):
    ...
    if not user:
        return
    olgaevent = await edit_or_reply(event, "**- جار تنزيل المستخدم من الاشراف**")
    ...
    try:
        ...
        return await olgaevent.edit(NO_PERM)
    await olgaevent.edit("**- تم بنجاح تنزيله من رتبة المشرفين**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"تنزيل مشرف\
...
async def _ban_person(event):
    ...
    olgaevent = await edit_or_reply(event, "- تم حظر المستخدم من الدردشة بنجاح")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    olgaevent = await edit_or_reply(event, "**- جار الغاء حظر المستخدم**")
    try:
        ...
        await olgaevent.edit("- لقد حدث خطأ اثناء الغاء حظر المستخدم")
    except Exception as e:
        await olgaevent.edit(f"**خطأ :**\n`{e}`")
...
async def kick(event):
    ...
    if not user:
        return
    olgaevent = await edit_or_reply(event, "**- جار طرد المستخدم من الدردشة**")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await olgaevent.edit(f"{NO_PERM}\n{e}")
    if reason:
        await olgaevent.edit(
            f"تم طرد [{user.first_name}](tg://user?id={user.id})\nالسبب: {reason}"
        )
    else:
        await olgaevent.edit(f"تم طرد[{user.first_name}](tg://user?id={user.id})")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "الطرد\n"
    ...
    if not flag:
        for msg in adminlog:
            ...
            if _media_type is None:
                ...
                deleted_msg += f"\n- {_media_type}\n**المرسل:**{_format.mentionuser(ruser.first_name ,ruser.id)}"
        await edit_or_reply(olgaevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(olgaevent, deleted_msg)
        for msg in adminlog:
            ruser = await event.client.get_entity(msg.old.from_id)
            _media_type = await media_type(msg.old)
            if _media_type is None:
