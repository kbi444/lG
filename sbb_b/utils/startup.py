...
LOGS = logging.getLogger("اعداد LG")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@sorsLG"))
        await sbb_b(JoinChannelRequest("@sorsLGg"))
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await sbb_b.send_message("@BotFather", botname)
            await asyncio.sleep(2)
            await sbb_b.send_message("@BotFather", "بُوت - مُسَاعِد LG")
            await asyncio.sleep(2)
            await sbb_b.send_message("@BotFather","/setuserpic")
            await asyncio.sleep(2)
            await sbb_b.send_message("@BotFather", botname)
            await asyncio.sleep(2)
            await sbb_b.send_file("@BotFather", file="razan/pic/bot.jpg")
            await asyncio.sleep(2)
            await sbb_b.send_message("@BotFather","/setdescription")
            await asyncio.sleep(2)
            await sbb_b.send_message("@BotFather", botname)
            await asyncio.sleep(2)
            await sbb_b.send_message("@BotFather", "- بُوت - مُسَاعِد LG .\n- لـ التوَاصُل مَع مَالك البُوت .\n- سوِي بُوت لَك مِن هِنا : @sorsLG")
            await asyncio.sleep(2)
            await sbb_b.send_message("@sorsLGG","- تم التنصيب ✔.")
            await asyncio.sleep(2)
        except Exception as e:
            print(e)
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.olgaLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/2af327cf756bd244e630d.jpg",
                caption="**- welcome to suorce LG .\n- to start send : `.الاوامر`",
                buttons=[(Button.url("قروب المساعدة", "https://t.me/sorsLGG"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كروب تحكم LG", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if olgaBRANCH := branch:
        repourl = os.path.join(olgaREPO, f"tree/{olgaBRANCH}")
        gcmd = f"git clone -b {olgaBRANCH} {olgaREPO} {cfolder}"
        errtext = f"لا يوحد فرع بأسم `{olgaBRANCH}` في الريبو الخارجي {olgaREPO}. تاكد من اسم الفرع عبر فار (`EXTERNAL_REPO_BRANCH`)"
    else:
        repourl = olgaREPO
        gcmd = f"git clone {olgaREPO} {cfolder}"
        errtext = f"الرابط ({olgaREPO}) الذي وضعته لفار `EXTERNAL_REPO` غير صحيح عليك وضع رابط صحيح"
